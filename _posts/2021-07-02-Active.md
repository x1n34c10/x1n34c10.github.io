...
Como son muchos puertos hice uso de la utilidad "extractports" de s4vitar para extraer los puertos y copiarlos en el portapeles.
...
Efectue otro escaneo con Nmap para detectar la version y servicio de cada puerto.
...
Al parecer estaba tratando con una maquina de Acive Directory, enumere los recursos compartidos con smbmap.
...
Tenia privilegios de lectura del recurso compartido Replication, accedi a el mediante smbcclient.
...
Habia un directorio llamado active.htb, tras a acceder a mutliples carpetas dentro de active.htb encontre un Groups.xml.
...
Me lo descargue con get para ver que tenia.
...
Encontre un la clave de preferencias de politica de grupo encriptada.
...
La clave de cifrado de Microsoft fue expuesta, por lo que la podiamos desencriptarla con `gpp-decrypt`.
...
Esta credencial me sirvio para acceder al recurso compartido **Users** mediante `smbcclient` haciendo uso del usuario **SVC_TGS**.
...
Ahora solo faltaba la escalada de privilegios, tenia Kerberos abierto, por lo que probe a dumpear el Tiquet de servicio del usuario Administrador mediante la herramienta `GetUserSPNs.py`.
...
Perfecto ya lo teniamos, le hice fuerza bruta con john the ripper y con un poco de suerte consegui desencriptarlo.
...
Probe a ver si esta credencial tenia acceso con `psexec.py` y al parecer si lo tenia.
