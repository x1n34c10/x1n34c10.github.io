...
layout: single
excerpt: "Esta es una máquina de dificultad fácil, la experiencia no ha sido frustrante por su facilidad. Haciendo fuzzing me encontré un directorio llamado cgi-bin por lo que pensé que podría tratarse de un shellshock, al volver a hacer fuzzing encontré un script llamado user.sh, y si era vulnerable a shellshock, una vez gane acceso a la máquina me aproveche de Perl porque podía ejecutar con sudo."
header:
show_date: true
classes: wide
header:
