...
layout: single
excerpt: "Esta es una máquina de dificultad fácil, la experiencia con esta máquina no ha sido frustrante por su facilidad. Haciendo fuzzing me encontré un directorio llamado cgi-bin por lo que pensé que podría tratarse de un shellshock, al volver a hacer fuzzing encontré un script llamado user.sh, y si era vulnerable a shellshock, una vez gane acceso a la máquina me aproveche de Perl que se podía ejecutar con sudo."
header:
show_date: true
classes: wide
header:
